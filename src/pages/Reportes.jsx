    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Resumen completo del negocio</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              ...
              <p className="text-2xl font-bold text-[#0F1F3D] mt-1">{datos.margenPromedio}%</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            {/* Ventas por vendedor */}
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <h3 className="text-sm font-semibold text-[#0F1F3D] mb-4">Ventas por vendedor</h3>
                  ...
                  {datos.ventasPorVendedor.map((v, i) => (
                    <div key={i}>
                      <div className="flex justify-between items-center mb-1">
                        <p className="text-sm text-gray-600">{v.nombre}</p>
                      ...
                      <div className="w-full bg-gray-100 rounded-full h-1.5">
                        <div
                          ...
                          style={{ width: `${Math.min((v.total / datos.ventasTotales) * 100, 100)}%` }}
                        />
                      </div>
                      <p className="text-xs text-gray-400 mt-1">{v.pedidos} pedido{v.pedidos > 1 ? 's' : ''}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Ranking clientes */}
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <h3 className="text-sm font-semibold text-[#0F1F3D] mb-4">Top clientes</h3>
                  ...
                  {datos.rankingClientes.map((c, i) => (
                    <div key={i} className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm text-gray-600 truncate max-w-28">{c.nombre}</p>
                      </div>
                      <p className="text-sm font-medium text-[#0F1F3D]">
              ...
              {datos.topProductos.length === 0 ? (
                ...
                <div className="space-y-2">
                  {datos.topProductos.map((p, i) => (
                    <div key={i} className="flex justify-between items-center">
                      <div>
                        <p className="text-sm text-gray-600 truncate max-w-32">{p.nombre}</p>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              ...
              <div className="space-y-2">
                {datos.stockValorizado.map((s, i) => (
                  <div key={i} className="flex justify-between items-center">
                    <p className="text-xs text-gray-600 truncate max-w-28">{s.nombre}</p>
                    <div className="text-right">
                      <p className="text-xs font-medium text-[#0F1F3D]">
                        ${s.valorVenta.toLocaleString('es-AR')}
                      </p>
              ...
              {datos.clientesConDeuda.length === 0 ? (
                ...
                <div className="space-y-2">
                  {datos.clientesConDeuda.map((c, i) => (
                    <div key={i} className="flex justify-between items-center">
                      <p className="text-sm text-gray-600 truncate max-w-32">{c.razon_social}</p>
